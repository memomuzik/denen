...
import config
from TS import LOGGER, app, userbot
from TS.core.call import Anony
from TS.misc import sudo
from TS.plugins import ALL_MODULES
from TS.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("TS.plugins" + all_module)
    LOGGER("TS.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Anony.start()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("TS").info("Stopping TS Music Bot...")
